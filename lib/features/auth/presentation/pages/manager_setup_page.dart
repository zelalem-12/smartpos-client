import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:go_router/go_router.dart';

import '../../../../core/constants/app_constants.dart';
import '../../../../core/router/app_router.dart';
import '../../../../core/theme/app_colors.dart';
import '../../domain/entities/user_entity.dart';
import '../../../../shared/widgets/app_button.dart';
import '../../../../shared/widgets/app_text_field.dart';
import '../cubit/manager_setup_cubit.dart';
import '../cubit/manager_setup_state.dart';
import '../widgets/pin_keypad_input.dart';

/// Manager setup screen — first-time creation of the manager account.
///
/// The manager enters their name, creates a 4-digit PIN, and confirms it.
/// On success, the app shows a confirmation card before navigating to login.
class ManagerSetupPage extends StatefulWidget {
  const ManagerSetupPage({super.key});

  @override
  State<ManagerSetupPage> createState() => _ManagerSetupPageState();
}

class _ManagerSetupPageState extends State<ManagerSetupPage> {
  final _nameController = TextEditingController();
  String _pin = '';
  String _confirmPin = '';
  int _step = 0; // 0 = create PIN, 1 = confirm PIN

  @override
  void dispose() {
    _nameController.dispose();
    super.dispose();
  }

  bool get _canSubmit =>
      _nameController.text.trim().isNotEmpty &&
      _pin.length == AppConstants.pinLength &&
      _confirmPin.length == AppConstants.pinLength;

  void _onPinCompleted(String pin) {
    setState(() {
      _pin = pin;
      _step = 1;
    });
  }

  void _onConfirmPinChanged(String confirmPin) {
    setState(() => _confirmPin = confirmPin);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColors.background,
      body: SafeArea(
        child: BlocConsumer<ManagerSetupCubit, ManagerSetupState>(
          listener: (context, state) {
            // Navigation is triggered manually from the success card.
          },
          builder: (context, state) {
            if (state is ManagerSetupSuccess) {
              return _buildSuccessView(context, state.manager);
            }

            final isLoading = state is ManagerSetupLoading;
            final errorText = state is ManagerSetupError ? state.message : null;

            return SingleChildScrollView(
              padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 32),
              child: ConstrainedBox(
                constraints: const BoxConstraints(maxWidth: 440),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildHeader(context),
                    const SizedBox(height: 24),
                    Card(
                      child: Padding(
                        padding: const EdgeInsets.all(24),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            AppTextField(
                              label: 'Manager Name',
                              hint: 'e.g. Abebe Bikila',
                              controller: _nameController,
                              enabled: !isLoading,
                              onChanged: (_) => setState(() {}),
                            ),
                            const SizedBox(height: 28),
                            _buildPinSection(
                              isLoading: isLoading,
                              errorText: errorText,
                            ),
                            const SizedBox(height: 24),
                            AppButton(
                              label: 'Create Manager',
                              isLoading: isLoading,
                              icon: Icons.person_add_outlined,
                              onPressed: _canSubmit && !isLoading
                                  ? () => context
                                      .read<ManagerSetupCubit>()
                                      .submit(
                                        name: _nameController.text,
                                        pin: _pin,
                                        confirmPin: _confirmPin,
                                      )
                                  : null,
                            ),
                            if (errorText != null) ...[
                              const SizedBox(height: 16),
                              _buildErrorText(context, errorText),
                            ],
                          ],
                        ),
                      ),
                    ),
                  ],
                ),
              ),
            );
          },
        ),
      ),
    );
  }

  Widget _buildHeader(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            Container(
              width: 44,
              height: 44,
              decoration: BoxDecoration(
                color: AppColors.primary,
                borderRadius: BorderRadius.circular(12),
              ),
              child: const Icon(
                Icons.person_add_alt_1,
                color: Colors.white,
                size: 24,
              ),
            ),
            const SizedBox(width: 14),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                    'Manager Setup',
                    style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                          fontWeight: FontWeight.w700,
                          color: AppColors.primary,
                        ),
                  ),
                  Text(
                    'Create the first manager account.',
                    style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                          color: AppColors.textSecondary,
                        ),
                  ),
                ],
              ),
            ),
          ],
        ),
      ],
    );
  }

  Widget _buildPinSection({
    required bool isLoading,
    String? errorText,
  }) {
    final creating = _step == 0;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
          creating ? 'Create 4-Digit PIN' : 'Confirm 4-Digit PIN',
          style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                fontWeight: FontWeight.w600,
              ),
        ),
        const SizedBox(height: 4),
        Text(
          creating
              ? 'Choose a secure 4-digit PIN for manager login.'
              : 'Re-enter the same PIN to confirm.',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
                color: AppColors.textSecondary,
              ),
        ),
        const SizedBox(height: 16),
        if (creating)
          PinKeypadInput(
            onCompleted: _onPinCompleted,
            enabled: !isLoading,
            hasError: errorText != null,
          )
        else
          PinKeypadInput(
            onChanged: _onConfirmPinChanged,
            enabled: !isLoading,
            hasError: errorText != null,
          ),
        if (!creating)
          Align(
            alignment: Alignment.centerRight,
            child: TextButton.icon(
              onPressed: isLoading
                  ? null
                  : () => setState(() {
                        _pin = '';
                        _confirmPin = '';
                        _step = 0;
                      }),
              icon: const Icon(Icons.refresh, size: 18),
              label: const Text('Start over'),
            ),
          ),
      ],
    );
  }

  Widget _buildErrorText(BuildContext context, String errorText) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: AppColors.error.withValues(alpha: 0.08),
        borderRadius: BorderRadius.circular(8),
      ),
      child: Row(
        children: [
          const Icon(Icons.error_outline, color: AppColors.error, size: 18),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              errorText,
              style: Theme.of(context)
                  .textTheme
                  .bodyMedium
                  ?.copyWith(color: AppColors.error),
            ),
          ),
        ],
      ),
    );
  }

  Widget _buildSuccessView(BuildContext context, UserEntity manager) {
    final theme = Theme.of(context);

    return Center(
      child: SingleChildScrollView(
        padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 32),
        child: ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 440),
          child: Card(
            child: Padding(
              padding: const EdgeInsets.all(32),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Container(
                    width: 72,
                    height: 72,
                    decoration: const BoxDecoration(
                      color: AppColors.success,
                      shape: BoxShape.circle,
                    ),
                    child: const Icon(
                      Icons.check,
                      color: Colors.white,
                      size: 40,
                    ),
                  ),
                  const SizedBox(height: 20),
                  Text(
                    'Manager Created',
                    style: theme.textTheme.titleLarge?.copyWith(
                          fontWeight: FontWeight.w700,
                          color: AppColors.primary,
                        ),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    'You can now log in with your 4-digit PIN.',
                    textAlign: TextAlign.center,
                    style: theme.textTheme.bodyMedium?.copyWith(
                          color: AppColors.textSecondary,
                        ),
                  ),
                  const SizedBox(height: 20),
                  Container(
                    padding: const EdgeInsets.symmetric(
                      horizontal: 20,
                      vertical: 14,
                    ),
                    decoration: BoxDecoration(
                      color: AppColors.background,
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: Row(
                      children: [
                        const Icon(
                          Icons.person_outline,
                          color: AppColors.primary,
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(
                                manager.name,
                                style: theme.textTheme.bodyLarge?.copyWith(
                                      fontWeight: FontWeight.w600,
                                    ),
                              ),
                              Text(
                                'Manager',
                                style: theme.textTheme.bodySmall?.copyWith(
                                      color: AppColors.textSecondary,
                                    ),
                              ),
                            ],
                          ),
                        ),
                      ],
                    ),
                  ),
                  const SizedBox(height: 28),
                  SizedBox(
                    width: double.infinity,
                    height: 52,
                    child: ElevatedButton.icon(
                      onPressed: () => context.go(AppRoutes.pinLogin),
                      icon: const Icon(Icons.login, size: 20),
                      label: const Text(
                        'Continue to Login',
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w600,
                        ),
                      ),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: AppColors.accent,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(12),
                        ),
                        elevation: 0,
                      ),
                    ),
                  ),
                ],
              ),
            ),
          ),
        ),
      ),
    );
  }
}
